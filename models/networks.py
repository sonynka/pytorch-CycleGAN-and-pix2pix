...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[], image_size=128):
    netG = None
...
def define_D(input_nc, ndf, which_model_netD,
    ...
    elif which_model_netD == 'stargan':
        netD = StarGANDiscriminator(input_nc, ndf, n_blocks=6, image_size=image_size)
    else:
...
class ClassificationLoss(nn.Module):
    ...
    def __call__(self, pred_label, real_label, batch_size):
        return self.loss(pred_label, real_label) / batch_size
...
class StarGANGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=6, image_size=128):
        ...
        layers.append(nn.InstanceNorm2d(ngf, affine=True))
...
class StarGANDiscriminator(nn.Module):
    ...
    def __init__(self, output_nc, ndf=64, n_blocks=6, image_size=128):
        ...
        k_size = int(image_size / np.power(2, n_blocks))
        self.main = nn.Sequential(*layers)
        self.conv1 = nn.Conv2d(curr_dim, 1, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv2 = nn.Conv2d(curr_dim, output_nc, kernel_size=k_size, bias=False)
